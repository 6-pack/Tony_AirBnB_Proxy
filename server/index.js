...
app.get('/rooms/:roomID/reviews', (req, res) => {
  axios.get('http://18.222.212.45:3001/rooms/2/reviews')
  .then(result => res.status(200).send(result.data))
  .catch(err => console.log(err));
});

app.get('/rooms/:roomID/reviews/:phrase', (req, res) => {
  axios.get(`http://18.222.212.45:3001/rooms/2/reviews/${req.params.phrase}`)
  .then(result => res.status(200).send(result.data))
  .catch(err => console.log(err));
});

app.get('/carousel', (req, res) => {
  axios.get(`http://18.144.126.16:3003/carousel`)
  .then(result => res.status(200).send(result.data))
  .catch(err => console.log(err));
});
